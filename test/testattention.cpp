...
int main() {
    ...
    for (auto& x : Q.data) x = static_cast<float>(rand()) / RAND_MAX;
    for (auto& x : K.data) x = static_cast<float>(rand()) / RAND_MAX;
    for (auto& x : V.data) x = static_cast<float>(rand()) / RAND_MAX;
