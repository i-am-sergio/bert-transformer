...
tuple<vector<Tensor>, vector<Tensor>, vector<Tensor>, vector<Tensor>> 
train_test_split(vector<Tensor>& X, vector<Tensor>& Y, 
                 float test_size = 0.2f, int random_seed = 42) {
    
    // Validar parámetros
    if (X.size() != Y.size()) {
        throw invalid_argument("X e Y deben tener el mismo número de muestras");
    }
    if (test_size <= 0.0f || test_size >= 1.0f) {
        throw invalid_argument("test_size debe estar entre 0 y 1");
    }

    // Mezclar los datos con la semilla proporcionada
    ...
    size_t split_idx = static_cast<size_t>(X.size() * (1.0f - test_size));
    ...
    Y_test.reserve(Y.size() - split_idx);
    ...
    cout << "Loaded:\n- train data: " << X_train.size() << " -> " << X_train[0].shape << endl;
    cout << "- valid data: " << X_test.size() << " -> " << X_test[0].shape << endl;
