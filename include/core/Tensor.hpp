...
private:
    ...
    void calculateStrides() {
        ...
        if (shape_.empty()) return; // Si no hay dimensiones, no hay strides
        strides_.back() = 1; // El stride de la última dimensión siempre es 1
        for (size_t i = shape_.size() - 1; i > 0; --i) { // Recorre las dimensiones de atrás hacia adelante
...
public:
    ...
    Tensor(const vector<size_t>& shape, const vector<float>& data) {
        ...
        size_t total_elements = 1;
        for (size_t dim_size : shape_) { // Calcula el número total de elementos
            total_elements *= dim_size;
        }
    ...
    float at(const vector<size_t>& indices) const { 
        ...
        for (size_t i = 0; i < shape_.size(); ++i) {
            if (indices[i] >= shape_[i]) { // Verifica si el índice está dentro de los límites
